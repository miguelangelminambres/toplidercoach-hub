...
async function ppCargarPartidosPendientes(){if(!clubId||!seasonId)return;var s=document.getElementById('pp-partido-select');if(!s)return;try{var{data:p,error}=await supabaseClient.from('matches').select('*').eq('club_id',clubId).eq('season_id',seasonId).order('match_date',{ascending:false});if(error)throw error;if(!p||!p.length){s.innerHTML='<option value="">-- Sin partidos --</option>';return}var h='<option value="">-- Elige partido --</option>';p.forEach(function(m){var f=new Date(m.match_date+'T12:00:00');var rTxt=m.result?(m.result==='win'?'✅ ':(m.result==='draw'?'🟡 ':'❌ ')):'📋 ';h+='<option value="'+m.id+'">'+rTxt+['Dom','Lun','Mar','Mie','Jue','Vie','Sab'][f.getDay()]+' '+f.getDate()+'/'+(f.getMonth()+1)+(m.kick_off_time?' '+m.kick_off_time.slice(0,5):'')+' — '+(m.home_away==='home'?'vs ':'@ ')+m.opponent+(m.competition?' · '+m.competition:'')+(m.result?' ('+(m.team_goals||0)+'-'+(m.opponent_goals||0)+')':'')+'</option>'});s.innerHTML=h}catch(e){showToast('Error: '+e.message)}}
...
async function ppSubirFotoJugador(input){var file=input.files[0];if(!file)return;try{var dataUrl=await ppComprimirImagen(file,200);_ppJugadorPhoto=dataUrl;var prev=document.getElementById('ppj-photo-preview');if(prev)prev.innerHTML='<img src="'+dataUrl+'" style="width:100%;height:100%;object-fit:cover">'}catch(e){showToast('Error: '+e.message)}}
function ppQuitarFotoJugador(){_ppJugadorPhoto='';var prev=document.getElementById('ppj-photo-preview');if(prev)prev.innerHTML='<span style="font-size:20px;color:#475569">📷</span>'}
